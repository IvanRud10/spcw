...
int ErrorChecking()
{
	...
	if (Data.LexTable[3].type != LSeparator)
	{
		Err_num++;
		fprintf(ef, "line %d: \t';' expected!\n", Data.LexTable[0].line);
	}

	if (Data.LexTable[4].type != LStart)
	{
		Err_num++;
	...
	{
		Err_num++;
		fprintf(ef, "line %d: \t'int16' expected!\n", Data.LexTable[3].line);
	}
	if (Data.LexTable[6].type == LVarType)
	{
		i = 7;
	...
	int brak = Balans(i, LEOF, LDo, LWhile);
	if (brak == 1)
	{
		Err_num++;
		fprintf(ef, "line %d: \tToo much 'while'!\n", Data.LexTable[j].line);
	}
	if (brak == 2)
	{
		Err_num++;
		fprintf(ef, "line %d: \t'while' expected!\n", Data.LexTable[j].line);
	}
