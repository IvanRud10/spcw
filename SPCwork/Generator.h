...
void PrintData(FILE* f)				//Print a data segment
{
	...
	fputs("\tInMessage\tdb\t\"In: \", 0\n", f);
	fputs("\tOutMessage\tdb\t\"Out: \",\" %d\",0\n", f);
...
int ConvertToPostfixForm(int i) //Forms in the array a sequence of token numbers that corresponds to the postfix form
{
	...
	for (z = 0; i < k; ++i)
	{
		...
		if (in == LRBraket)
		{
			...
			{
				Data.bufExprPostfixForm[z] = Stack.Pop(&Stack.S);	//stackStack.S.st[stackStack.S.top];
				++z;
			}
			Stack.Pop(&Stack.S);
			Data.bufExprPostfixForm[z] = i;
	...
	for (n = 0; Data.bufExprPostfixForm[n] != 3000; ++n)
	{
		...
		if ((!IsOperation(Data.LexTable[Data.bufExprPostfixForm[n]].type)) && (Data.LexTable[Data.bufExprPostfixForm[n]].type != LNot) && (Data.LexTable[Data.bufExprPostfixForm[n - 1]].type != LNot))
		{
			...
			else if (Data.LexTable[Data.bufExprPostfixForm[n]].type == LNumber)
			{
				...
				//fprintf(f, "\tmov word ptr buf,%d\n", Data.LexTable[Data.bufExprPostfixForm[n]].value);
				fputs("\tfild buf\n", f);
